...
check("six providers registered", names == {"hetzner", "digitalocean", "aws", "azure", "gcp", "vultr"}, str(sorted(names)))
...
check("vultr spec declares a token field", any(s["name"] == "vultr" and any(f["name"] == "token" for f in s["fields"]) for s in specs))

print("-- v1.5: custom actions --")
st, ca = call("POST", "/api/custom-actions", {"label": "e2e echo", "category": "custom", "command": "echo hello-{{msg}}", "params": [{"name": "msg", "label": "Message"}], "requires": [], "sudo": "", "danger": 0})
check("create custom action", st == 201 and ca["id"] == "custom.e2e-echo", f"{st} {ca}")
st, r = call("POST", "/api/custom-actions", {"label": "bad", "command": "rm -rf /", "params": [], "requires": [], "sudo": "", "danger": 3})
check("tier-3 custom action refused at creation", st == 400)
st, r = call("POST", "/api/custom-actions", {"label": "unused", "command": "echo hi", "params": [{"name": "x", "label": "X"}], "requires": [], "sudo": "", "danger": 0})
check("param not used in command refused", st == 400)
st, acts = call("GET", f"/api/machines/{mid}/actions")
check("custom action offered on the host", any(a["id"] == "custom.e2e-echo" for a in acts))
st, r = call("POST", f"/api/machines/{mid}/actions/custom.e2e-echo", {"params": {"msg": "world"}, "confirm": False, "confirmName": ""})
check("custom action runs and echoes the quoted param", st == 200 and "hello-world" in r["data"]["text"])
st, r = call("POST", f"/api/machines/{mid}/actions/custom.e2e-echo", {"params": {"msg": "a\nb"}, "confirm": False, "confirmName": ""})
check("control char in custom param -> 400", st == 400)
st, r = call("POST", f"/api/machines/{mid}/actions/custom.e2e-echo", {"params": {"msg": "x" * 600}, "confirm": False, "confirmName": ""})
check("over-long custom param -> 400", st == 400)

print("-- v1.5: groups --")
st, g = call("POST", "/api/groups", {"name": "e2e grp", "tags": ["e2e"]})
check("create group", st == 201); gid = g["id"]
st, r = call("POST", "/api/groups", {"name": "empty", "tags": []})
check("tagless group refused", st == 400)
st, gs = call("GET", "/api/groups")
check("group resolves 1 member (machine is tagged e2e)", any(x["id"] == gid and x["members"] == 1 for x in gs))
st, gm = call("GET", f"/api/groups/{gid}/machines")
check("group machines returns the tagged machine", len(gm) == 1 and gm[0]["id"] == mid)

print("-- v1.5: bulk --")
st, r = call("POST", "/api/bulk/actions/system.overview", {"machineIds": [mid]})
check("bulk read needs a confirm first (428 + preview)", st == 428 and r["code"] == "needs_confirm" and r["details"]["count"] == 1)
st, r = call("POST", "/api/bulk/actions/system.overview", {"machineIds": [mid], "confirm": True})
check("bulk read runs after confirm", st == 200 and r["count"] == 1 and r["results"][0]["ok"])
st, cw = call("POST", "/api/custom-actions", {"label": "e2e noop", "command": "true", "params": [], "requires": [], "sudo": "", "danger": 1})
check("create tier-1 custom action for bulk", st == 201)
st, r = call("POST", "/api/bulk/actions/custom.e2e-noop", {"groupId": gid})
check("bulk write needs the host count typed (428 needs_count)", st == 428 and r["code"] == "needs_count" and r["details"]["count"] == 1)
st, r = call("POST", "/api/bulk/actions/custom.e2e-noop", {"groupId": gid, "confirmText": "wrong"})
check("bulk write wrong count still refused", st == 428)
st, r = call("POST", "/api/bulk/actions/custom.e2e-noop", {"groupId": gid, "confirmText": "1"})
check("bulk write runs when count matches", st == 200 and r["results"][0]["ok"])

# cleanup v1.5 fixtures so the audit-count assertions below stay stable-ish
call("DELETE", f"/api/groups/{gid}")
call("DELETE", "/api/custom-actions/custom.e2e-echo")
call("DELETE", "/api/custom-actions/custom.e2e-noop")
